<script lang="ts">
	...
	let evaluatedArea = $derived(
		`A = \\frac{1}{2}(\\text{${formatNumber(a)}} + \\text{${formatNumber(b)}}) \\times \\text{${formatNumber(height)}} = \\text{${formatNumber(displayArea)}} \\text{ ${unit}}^2`
	);
	...
	let areaSteps = $derived([
		...
		`A = \\frac{1}{2} \\times (\\text{${formatNumber(a)}} + \\text{${formatNumber(b)}}) \\times \\text{${formatNumber(height)}}`,
	...
	function handleUnitChange(oldUnit: Unit, newUnit: Unit) {
		if (oldUnit !== newUnit) {
			a = convertValue(a, oldUnit, newUnit);
			b = convertValue(b, oldUnit, newUnit);
...
<Breadcrumb
	items={[
		...
		{ label: tKey('shapes.trapezoid.name') }
	]}
/>
...
<div class="mb-8">
	...
	<h1 class="font-display text-4xl font-bold tracking-tight text-text-primary">
		{tKey('shapes.trapezoid.name')}
	</h1>
	<p class="mt-2 text-text-secondary">{tKey('shapes.trapezoid.desc')}</p>
</div>
...
<div class="grid grid-cols-1 items-start gap-6 lg:grid-cols-2">
	...
	<div class="glow-panel flex aspect-square items-center justify-center p-8">
		<svg viewBox="0 0 200 200" class="shape-glow h-full w-full max-w-xs">
			...
			<text
				...
				font-size="11"
				fill="#475569"
				text-anchor="middle">a = {formatNumber(a)}</text
			>
			<text
				x="100"
				y={70 + svgHeight + 14}
				...
				font-size="11"
				fill="#475569"
				text-anchor="middle">b = {formatNumber(b)}</text
			>
			<text
				x="95"
				y={70 + svgHeight / 2}
	...
	<div class="surface-panel flex flex-col gap-6 p-6">
		...
		<div class="flex flex-col gap-4">
			...
			<SliderInput
				label={tKey('shapes.trapezoid.baseA')}
				bind:value={a}
				min={0}
				max={100}
				step={1}
			...
			/>
			<SliderInput
				label={tKey('shapes.trapezoid.baseB')}
				bind:value={b}
				min={0}
				max={100}
				step={1}
			...
			/>
			<SliderInput
				label={tKey('shapes.trapezoid.height')}
				bind:value={height}
				min={0}
				max={100}
				step={1}
		...
		<CalculationHistory shapeId="trapezoid" />
	</div>
</div>
