<script lang="ts">
	...
	const pageData = mathSolverData({
		shape: 'Trapezoid',
		urlPath: '/2d/trapezoid',
		category: '2D',
		resultType: 'area and perimeter',
		keywords: 'trapezoid area calculator, trapezoid perimeter, bases height, quadrilateral math'
	});
	...
	let topCenter = $derived(100 - (svgB - svgA) / 2);

	let evaluatedArea = $derived(
		`A = \\frac{1}{2}(\\text{${formatNumber(a)}} + \\text{${formatNumber(b)}}) \\times \\text{${formatNumber(height)}} = \\text{${formatNumber(result.area)}} \\text{ ${unit}}^2`
	);
	...
	onMount(() => {
		...
		if (pa !== null) {
			...
			if (!isNaN(val) && val > 0) a = val;
		}
		const pb = sp.get('b');
		if (pb !== null) {
			const val = parseFloat(pb);
			if (!isNaN(val) && val > 0) b = val;
		}
		const ph = sp.get('h');
		if (ph !== null) {
...
<SeoHead
	title="Trapezoid Calculator — Area & Perimeter"
	description="Interactive trapezoid calculator. Calculate area and perimeter with step-by-step formulas, real-time diagrams, and instant results. Free — no signup."
	canonicalPath="/2d/trapezoid"
	keywords="trapezoid area calculator, trapezoid perimeter, bases height, quadrilateral math"
	structuredData={pageData}
/>

<Breadcrumb items={[{ label: 'Home', href: '/' }, { label: '2D Geometry', href: '/2d' }, { label: 'Trapezoid' }]} />

<BackButton href="/2d" />

<div class="mb-8">
	<p class="micro-label mb-2">2D Geometry</p>
	<h1 class="font-display font-bold text-4xl tracking-tight text-text-primary">Trapezoid Calculator</h1>
	<p class="text-text-secondary mt-2">Calculate area and approximate perimeter of a trapezoid.</p>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
	...
	<div class="glow-panel aspect-square flex items-center justify-center p-8">
		<svg viewBox="0 0 200 200" class="shape-glow w-full h-full max-w-xs">
			<polygon
				points={`${topCenter},70 ${topCenter + svgA},70 ${100 + svgB / 2},${70 + svgHeight} ${100 - svgB / 2},${70 + svgHeight}`}
				fill="rgba(99,102,241,0.08)"
				stroke="#818CF8"
				stroke-width="2"
			/>
			<line x1="100" y1="70" x2="100" y2={70 + svgHeight}
				stroke="rgba(148, 163, 184, 0.4)" stroke-width="1" stroke-dasharray="4 3"/>
			<text x="100" y="65" font-family="JetBrains Mono" font-size="11" fill="#475569" text-anchor="middle">a = {formatNumber(a)}</text>
			<text x="100" y={70 + svgHeight + 14} font-family="JetBrains Mono" font-size="11" fill="#475569" text-anchor="middle">b = {formatNumber(b)}</text>
			<text x="95" y={70 + svgHeight / 2} font-family="JetBrains Mono" font-size="11" fill="#475569" text-anchor="end">h = {formatNumber(height)}</text>
		</svg>
	</div>
	...
	<div class="surface-panel p-6 flex flex-col gap-6">
		...
		<div class="flex flex-col gap-4">
			...
			<div class="flex flex-col gap-1.5">
				<label class="micro-label" for="a">Top Base (a)</label>
				<div class="relative">
					<input id="a" type="number" min="0" class="w-full bg-bg-inset border border-border-default rounded-lg px-4 py-2.5 font-mono text-text-primary placeholder:text-text-muted focus:outline-none focus:border-border-strong focus:ring-2 focus:ring-indigo/20 transition-colors duration-150" placeholder="Enter a positive number" bind:value={a} onkeydown={handleKeyDown} />
					<span class="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted text-sm font-mono">{unit}</span>
				</div>
			</div>
			<div class="flex flex-col gap-1.5">
				<label class="micro-label" for="b">Bottom Base (b)</label>
				<div class="relative">
					<input id="b" type="number" min="0" class="w-full bg-bg-inset border border-border-default rounded-lg px-4 py-2.5 font-mono text-text-primary placeholder:text-text-muted focus:outline-none focus:border-border-strong focus:ring-2 focus:ring-indigo/20 transition-colors duration-150" placeholder="Enter a positive number" bind:value={b} onkeydown={handleKeyDown} />
					<span class="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted text-sm font-mono">{unit}</span>
				</div>
			</div>
			<div class="flex flex-col gap-1.5">
...
<ShapeFacts shapeId="trapezoid" />
<ShapeProperties shapeId="trapezoid" />
